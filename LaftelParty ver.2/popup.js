document.addEventListener('DOMContentLoaded', async () => {
    ...
    chrome.runtime.sendMessage({ type: "GET_STATUS" }, (info) => {
        if (info) {
            //showStatus(info.url, info.room, info.role, info.lastTime, info.isPlaying, info.title, info.episode); //room제거
            showStatus(info.URL, info.role, info.lastTime, info.isPlaying, info.title, info.episode);
        }
    });
    //팝업창에 상태 표시
    ...
    */
    const showStatus = (URL, role, lastTime = 0, isPlaying, title, episode) => {
        inputArea.style.display = 'none';
        statusArea.style.display = 'block';

        document.getElementById('display-url').innerText = URL //초대 URL 표기;
        //document.getElementById('display-room').innerText = room;
    ...
    const handleConnection = (type) => {
        ...
        chrome.runtime.sendMessage(data, (response) => {
            if (response && response.status === "success") {
                //showStatus(data.serverUrl, data.room, response.role, data.lastTime, data.isPlaying, data.title, data.episode);->room 없애기

                showStatus(data.URL, response.role, data.lastTime, data.isPlaying, data.title, data.episode);

                msgArea.innerText = "성공적으로 연결되었습니다!";
            } else {
    ...
    const handleCopy = () => {
        const inviteURL = document.getElementById('display-url').innerText;

        navigator.clipboard.writeText(inviteURL).then(() => {
            alert("초대 URL이 클립보드에 복사되었습니다!");
        }).catch(err => {
            console.error('초대 URL 복사 실패:', err);
        });
    ...
    document.getElementById('URLCopyBtn').addEventListener('click', () => { handleCopy() }); // 복사 버튼 클릭 시 handleCopy 함수 호출
